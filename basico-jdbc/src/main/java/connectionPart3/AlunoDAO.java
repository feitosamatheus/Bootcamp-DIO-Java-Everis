...
public class AlunoDAO {
    ...
    public List<Aluno> list() {
        ...
        try (Connection conn = ConnectionFactory.getConnection()) {
            String sql = "SELECT * FROM aluno"; // Preparar consulta SQL.

            PreparedStatement stmt = conn.prepareStatement(sql);// <-- Preparar statement com os parâmetros recebidos (nesta função não tem
            // parâmetros, pois irá retornar todos os valores da tabela aluno)

            ResultSet rs = stmt.executeQuery();                 //<-- Executa consulta e armazena o retorno da consulta no objeto "rs".

            while (rs.next()) {                                    //<-- Criar um objeto aluno e guardar na lista de alunos.
